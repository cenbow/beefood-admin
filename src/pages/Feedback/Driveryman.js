...
@Form.create()
class DeliverymanFeedbackList extends PureComponent {
  state = {
    modalVisible: false,
  ...
  fetchMenu = () => {
    ...
    dispatch({
      ...
      callback: res => {
        if (res.code == 200) {
          this.setState({
            menu: getMenu(res.data.items, 'feedback', 'driveryman'),
          });
        }
      },
    });
  ...
  getList = params => {
    ...
    // 获取订单列表
    dispatch({
      type: 'feedback/fetchDeliverymanFeedbackList',
      payload: getParams,
    });
  };
  ...
  handleUpdate = fields => {
    ...
    dispatch({
      type: 'feedback/replyDeliverymanFeedback',
      payload: formData,
      callback: res => {
  ...
  handleDelete = fields => {
    ...
    confirm({
      ...
      onOk: () => {
        dispatch({
          type: 'feedback/removeDeliverymanFeedback',
          payload: formData,
          callback: res => {
            if (!utils.successReturn(res)) return;
  ...
  render() {
    const {
      feedback: { deliverymanFeedbackList },
      loading,
    } = this.props;
    ...
    return (
      <div>
        ...
        <div className="page_head">
            <span className="page_head_title">骑手意见反馈</span>
          </div>
...
export default DeliverymanFeedbackList;
