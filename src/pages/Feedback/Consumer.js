...
@Form.create()
class ConsumerFeedbackList extends PureComponent {
  state = {
    modalVisible: false,
  ...
  fetchMenu = () => {
    ...
    dispatch({
      ...
      callback: res => {
        if (res.code == 200) {
          this.setState({
            menu: getMenu(res.data.items, 'feedback', 'consumer'),
          });
        }
      },
    });
  ...
  getList = params => {
    ...
    // 获取订单列表
    dispatch({
      type: 'feedback/fetchUserFeedbackList',
      payload: getParams,
    });
  };
  ...
  handleUpdate = fields => {
    ...
    dispatch({
      type: 'feedback/replyUserFeedback',
      payload: formData,
      callback: res => {
  ...
  handleDelete = fields => {
    ...
    confirm({
      ...
      onOk: () => {
        dispatch({
          type: 'feedback/removeUserFeedback',
          payload: formData,
          callback: res => {
            if (!utils.successReturn(res)) return;
      ...
      <div>
        <Card bordered={false}>
          <div className="page_head">
            <span className="page_head_title">用户意见反馈</span>
          </div>
...
export default ConsumerFeedbackList;
