...
const CreateForm = Form.create()(props => {
  ...
  return (
    <Card type="inner" title={<span><Icon type="message" /> 用户消息推送</span>}>
      ...
      <FormItem {...formLayout} label="柬文内容">
        ...
        >
          ...
          {menu.indexOf('historyPush') != -1 ? (
            <Link style={{ marginLeft: 8 }} to="/message/consumer/history">
              <Button icon="menu">
...
@connect(({ messageConsumerList, common, loading }) => ({
  messageConsumerList,
  common,
  loading: loading.models.messageConsumerList,
}))
@Form.create()
class MessageList extends PureComponent {
  ...
  columns = [
    ...
    },
    {
      ...
      render: val => (utils.deteFormat(val))
    },
    {
      title: '状态',
      render: (text, record) => (
  ...
  fetchMenu = () => {
    ...
    dispatch({
      ...
      callback: res => {
        if (res.code == 200) {
          this.setState({
            menu: getMenu(res.data.items, 'message', 'consumer'),
          });
        }
      },
    });
  ...
  getList = params => {
    ...
    dispatch({
      type: 'messageConsumerList/fetchConsumerList',
      payload: getParams,
    });
  };
  ...
  handleSubmitMessage = (fields, form) => {
    ...
    dispatch({
      type: 'messageConsumerList/fetchMessageConsumerSend',
      payload: formData,
      callback: res => {
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <div className="page_head">
          <span className="page_head_title">用户列表</span>
        </div>
