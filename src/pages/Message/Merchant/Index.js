...
const CreateForm = Form.create()(props => {
  ...
  return (
    <Card type="inner" title={<span><Icon type="message" /> 商家消息推送</span>}>
      ...
      <FormItem {...formLayout} label="柬文内容">
        ...
        >
          ...
          {menu.indexOf('historyPush') != -1 ? (
            <Link style={{ marginLeft: 8 }} to="/message/merchant/history">
              <Button icon="menu">
...
@connect(({ messageMerchantList, common, loading }) => ({
  messageMerchantList,
  common,
...
class MessageList extends PureComponent {
  ...
  columns = [
      ...
      render: (text, record) => (
        <div>
          <Badge
            status={
              statusMap[
              record.user_audit != null ? record.user_audit.verify_status : 0
              ]
            }
            text={
              verify_status[
    ...
    {
      title: '营业状态',
      dataIndex: 'm_user_info.business_type',
  ...
  fetchMenu = () => {
    ...
    dispatch({
      ...
      callback: res => {
        if (res.code == 200) {
          this.setState({
            menu: getMenu(res.data.items, 'message', 'merchant'),
          });
        }
      },
    });
  ...
  getList = params => {
    ...
    dispatch({
      type: 'messageMerchantList/fetchMerchantList',
      payload: getParams,
    });
  };
  ...
  handleSubmitMessage = (fields, form) => {
    ...
    dispatch({
      type: 'messageMerchantList/fetchMessageMerchantSend',
      payload: formData,
      callback: res => {
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <div className="page_head">
          <span className="page_head_title">商家列表</span>
        </div>
